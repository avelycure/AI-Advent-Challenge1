...
PROVIDERS: Dict[str, ProviderInfo] = {
    ...
    "openrouter": ProviderInfo(
        ...
        # Каждая проверена живым запросом: каталог провайдера меняется, и модели
        # из него пропадают — прежние minimax-m3 и glm-5.2 отвечали уже кодом 404.
        models=[
            ModelInfo("nex-agi/nex-n2.5-pro:free", "Nex N2.5 Pro — отвечает по существу и сразу",
                      262_144, 235_929),
            ModelInfo("nvidia/nemotron-3.5-lightning:free",
                      "Nemotron 3.5 Lightning — окно на миллион токенов, рассуждает перед ответом",
                      1_000_000, 65_536),
            # Самое узкое окно среди бесплатных моделей каталога. Нужно затем,
            # чтобы увидеть отказ самого провайдера: у остальных окно так велико,
            # что до переполнения не добраться ни вставкой, ни разговором.
            ModelInfo("liquid/lfm-2.5-2.6b:free",
                      "LFM 2.5 2.6B — окно всего 65 536 токенов, на нём видно переполнение",
                      65_536, 8_192),
        ],
        notes=["Один ключ открывает модели разных разработчиков. Все перечисленные "
               ...
               "«перегружена». Nex отвечает коротко и по делу; Nemotron и LFM "
               "рассуждают перед ответом, и на короткий вопрос у них уходит около "
               "сотни токенов выхода — с маленьким max_tokens текста не будет вовсе.",
               "LFM держит окно в 65 536 токенов и на запрос шире отвечает "
               "настоящим отказом: «This endpoint's maximum context length is "
               "65536 tokens». Прочие модели каталога слишком просторны, чтобы "
               "переполнить их вручную."],
        free=True,
    ),
    "groq": ProviderInfo(
        key="groq",
