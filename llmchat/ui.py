...
def read_user_line(console: Console, prompt: str) -> str:
    ...
    Перевод строки внутри вставки для ``input()`` неотличим от нажатия Enter,
    поэтому многострочная вставка уходила бы в модель по строке за запрос.
    Чтобы этого не было, у терминала спрашивается «скобочная вставка»: он
    обрамляет вставленный текст особыми последовательностями и тем прямо
    говорит, где вставка началась и где кончилась.

    Догадываться нельзя. Прежняя версия считала вставкой всё, что оказалось во
    входной очереди сразу после Enter, — «человек так быстро не напечатает».
    Но пока модель думает, человек успевает набрать следующую строку, и она
    попадала в ту же очередь: сообщения склеивались, а набранные слова
...
# читал сам Python. Обкусанным — если её читал libedit из macOS: он съедает
# начало управляющей последовательности и оставляет в строке «200~» или даже
# просто «00~». Измерено на живом терминале, а не предположено.
PASTE_OPEN = re.compile(r"^(?:\x1b\[200~|200~|00~)")
...
# только после того, как буфер освободится.
PASTE_IDLE_TENTHS = 3


def enable_bracketed_paste() -> None:
    """Попросить терминал обрамлять вставку и вернуть всё как было при выходе.

    Без обрамления многострочная вставка станет несколькими сообщениями —
    неудобно, но честно. Прежняя версия обрамление наоборот отключала и
    угадывала вставку по времени, а угадывание портило набранный текст.
    """
...
def _disable_bracketed_paste() -> None:
    try:
        ...
        sys.stdout.flush()
    except Exception:  # noqa: BLE001 — на выходе шуметь нечем и незачем
        pass


def _paste_started(text: str) -> bool:
    """Строка начинается обрамлением вставки.

    Привязка к началу строки обязательна: обкусанное обрамление вида «00~»
    коротко, и без привязки за него можно было бы принять обычный текст.
    """
    return bool(PASTE_OPEN.match(text))


def _paste_ended(text: str) -> bool:
    return bool(PASTE_CLOSE.search(text))
...
def _paste_tail() -> str:
    """Остаток начатой вставки — до её закрывающего обрамления.

    Читать обычным способом нельзя: в каноническом режиме терминал придерживает
    незавершённую строку до Enter, и она не видна ни ``select``, ни ``read``.
    Поэтому режим на мгновение снимается. Делается это только внутри вставки,
    о начале которой сказал сам терминал.
    """
    ...
    try:
        ...
        while True:
            ...
            if not data:
                break
            chunks.append(data)
            # Дочитывать после закрывающего обрамления нечего: дальше уже
            # обычный набор, и забирать его нельзя.
            if PASTE_CLOSE.search(b"".join(chunks).decode("utf-8", "replace")):
                break
    except OSError:
        pass
    finally:
