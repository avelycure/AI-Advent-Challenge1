...
def build_scenario(rec: TerminalRecorder, key: str, provider: str, model: str) -> None:
    ...
    steps = [
        ...
        {"caption": "Способ 3 — модель сначала пишет промпт себе",
         "inner": [("Промпт, который модель написала себе сама", 13.0,
        ...
        {"caption": "Способ 4 — три эксперта отвечают независимо, каждый своим запросом",
         "inner": [("Инженер-математик", 8.5, "Эксперт 1 — инженер-математик"),
                   ("Учитель литературы", 8.5, "Эксперт 2 — учитель литературы"),
                   ("Медик-инфекционист", 8.5, "Эксперт 3 — медик-инфекционист")],
         "hold": 10.0},
    ]
